...
<section class="table-container">
    <div class="table-section-header">
        <h3>Links do Episódio</h3>
        <a href="{{ route('episodes.links.create', ['episode' => $episode->id]) }}" class="btn btn-primary btn-sm">
        ...
        <tbody>
            @forelse ($links as $link)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('episodes.links.edit', ['link' => $link->id]) }}" 
                           class="btn btn-primary btn-sm" title="Editar">
                            <i class="fas fa-edit"></i>
                        </a>

                        <form action="{{ route('episodes.links.destroy', ['link' => $link->id]) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" 
